...
func ParseDefine(tpl string) (tplDefines TPLDefines, err error) {
	...
	delimLen := len(delim)
	tpl = funcs.TrimSpaces(tpl) // 去除开头结尾的非有效字符
...
func (d *TPLDefine) NameCamel() (nameCamel string) {
	nameCamel = funcs.ToCamel(d.Name)
	return
}
...
func (d *TPLDefine) parseContent() (err error) {
	content := funcs.TrimSpaces(d.Text) // 去除开头结尾的非有效字符
	...
	content = funcs.TrimSpaces(content)
	d.Content = content
	return nil
}
...
func (d *TPLDefine) ContentFirstLine(s string) (firstLine string) {
	...
	for {
		...
		firstLine = funcs.TrimSpaces(firstLine)
		if firstLine != "" {
			break
		}
...
func (dl TPLDefines) IsDefineNameCamel(variableName string) bool {
	for _, TPLDefine := range dl {
		if funcs.ToCamel(TPLDefine.Name) == variableName {
			return true
		}
	}
	return false
