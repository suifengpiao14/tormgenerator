...
{{end}}
		...
		`
		tplDefine, err := newTPLDefine(tormTpl)
		require.NoError(t, err)
		table := tplDefine.GetTable()
		fmt.Println(table)
	})
	t.Run("form2", func(t *testing.T) {
		tormTpl := "{{define \"ServiceGetByServiceID\"}} select * from `t_service`  where `service_id`=:ServiceID  and `deleted_at` is null; {{end}}"
		tplDefine, err := newTPLDefine(tormTpl)
		require.NoError(t, err)
		table := tplDefine.GetTable()
		fmt.Println(table)
...
{{end}}
	...
	t.Run("update2", func(t *testing.T) {
		tormTpl := "{{define \"ServiceDel\"}}update `t_service` set `deleted_at`={{currentTime .}} where `service_id`=:ServiceID;{{end}}"
		tplDefine, err := newTPLDefine(tormTpl)
		require.NoError(t, err)
		table := tplDefine.GetTable()
		fmt.Println(table)
